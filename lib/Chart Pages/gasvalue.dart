...
class GasSensor extends StatefulWidget {
  GasSensor({Key key, this.tier, this.userId, this.device})
    : super(key: key);
  ...
  @override
  State<StatefulWidget> createState() => new _GasSensorState();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: new AppBar(
        ...
        centerTitle: true,
        title: Text('GasSensor', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
        actions: <Widget>[
          Padding(
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        if(!snapshot.hasData) {
          ...
          List<GasSensorData> gasval = snapshot.data.documents.map((documentSnapshot) => GasSensorData.fromMap(documentSnapshot.data)).toList();
          return _buildChart(context, gasval);
        }
      }
    );
  }

  Widget _buildChart(context, gasval) {
    return Padding(
      ...
      child: Container(
        child: Center(
          child: Column(
            children: <Widget> [
              SizedBox(height: 20,),
              Text('GasSensor Value of $device', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w300),),
              SizedBox(height: 20,),
              FadeAnimation(1, Container(
                  ...
                  child: SfCartesianChart(
                    ...
                    series: <ChartSeries>[
                      SplineSeries<GasSensorData, DateTime>(
                        dataSource: gasval,
                        splineType: SplineType.monotonic,
                        // cardinalSplineTension: 0.9,
                        xValueMapper: (GasSensorData gasvalData, _) => gasvalData.time,
                        yValueMapper: (GasSensorData gasvalData, _) => gasvalData.gasval,
                        name: 'GasSensor Value',
...
class GasSensorData {
  ...
  GasSensorData(this.time, this.gasval);

  GasSensorData.fromMap(Map<String, dynamic> map)
  :assert(map['Temperature'] != null),
  assert(map['Time'] != null),
    gasval = map['Temperature'],
    time = map['Time'].toDate();

  @override
   String toString() => "Record<$gasval:$time>";
